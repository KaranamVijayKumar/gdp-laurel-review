...
if (($handle = fopen($fileName, "r")) !== FALSE) {
    ...
    while (($data = fgetcsv($handle, 1000, ",")) !== FALSE) {
        ...
        if ($data[0] && $data[1] && $data[2] && $data[3]) {
			...
			$sqlInsert4 = "INSERT INTO `issue_toc_contents` (`id`, `issue_id`, `issue_toc_title_id`, `slug`, `status`, `highlight`, `content`, `content_text`, `created`, `modified`) VALUES (NULL, '" . $issue_id . "', '" . $lastTOCinsert_id . "', '" . $slug . "', '1', '1', '" . addslashes($page->getText()) . "', '" . addslashes($data[3]) . "', UNIX_TIMESTAMP(), NULL)";
