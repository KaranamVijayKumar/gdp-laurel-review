 ...
 */
class Aboutuss extends ORM
{

    /**
     * @var string
     */
    protected static $table = 'dev_aboutus';
     ...
     * @var string
     */
    protected static $cache_prefix = 'aboutus';
...
//echo static::ABOUTUS_IMG_PATH . $newImage_file; exit; 

                    if (move_uploaded_file($_FILES["image_file"]["tmp_name"], static::ABOUTUS_IMG_PATH . $newImage_file)) {
                        $model->set(
                            array(
                                'slug' => $input['slug'],
                                'description' => $input['description'],
                                'content' => $input['content'],
                                'insta_link' => $input['instalink'],
                                'fb_link' => $input['fbLink'],
                                'twitter_link' => $input['twitter_link'],
                                'wordpress_link' => $input['wordPress'],
    ...
    public function updateFromForm(array $input)
    {
        ...
        if ($validator->validate()) {
            try {
                ...
                $input = $validator->data();
                $content_text = new Html2Text($input['content']);
                $podcast = Aboutuss::findOrFail((int)$input['id']);
                ...
                if ($_FILES["image_file"]["name"]) {
                    ...
                    // $allowed_file_types = array('.jpg','.jpeg','.png');
                    $newImage_file = trim(str_replace('.', '', $filename1)) . rand() . $file_ext;
                    move_uploaded_file($_FILES["image_file"]["tmp_name"], static::ABOUTUS_IMG_PATH . $newImage_file);
