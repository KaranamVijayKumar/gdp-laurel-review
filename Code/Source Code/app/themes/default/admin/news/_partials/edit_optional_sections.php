...
if (count($optional_sections)) {
    ...
    ?>
    <h4 class="layout__item 1/1 mt content-hero">
    ...
    <?php
        foreach ($optional_sections as $key => $section) { ?>
            ...
            echo post('optional-section-' . $section) || array_key_exists(
                'optional-section-' . $section,
                $errors
            ) || (isset($article->sections[$section]) && $article->sections[$section]->content) ? '' : 'hidden' ?>"
                 id="os-<?php echo $section ?>">
                <?php
                ...
                echo \Story\Form::textarea(
                    'optional-section-' . $section,
                    isset($article->sections[$section]) ? $article->sections[$section]->attributes['content'] : '',
                    array(
                        'rows'                     => '20',
