<?php
if (has_access('manage_admin_orders_export')) {
    ?>
    <hr class="mb0"/>
    <div class="content-hero pv--">
        <form action="<?php
        echo action('\Project\Controllers\Admin\Orders\Export') ?>" method="post" class="filter">
            ...
            <p class="mb0 mt-- gray">
                <small>
                    <?php
                    echo sprintf(
                        _('These exports are limited to %s orders.'),
                        \Project\Services\Exporter\Exporters\OrderExporter::LIMIT
                    ) ?>
                </small>
            </p>
            <?php echo \Story\Form::hidden('status', $selectedOrderStatus) ?>
