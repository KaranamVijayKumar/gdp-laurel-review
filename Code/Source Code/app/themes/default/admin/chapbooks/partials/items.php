<div id="results">
    ...
    <?php } else { ?>
        <p class="mb0"><em><?php echo sprintf(ngettext('%d chapbook.', '%d chapbooks.', $total), $total) ?></em></p>
        <ol class="item-list pt">
            <?php foreach ($items as $item) { ?>
                <?php
                $url =  action('\Project\Controllers\Admin\Chapbooks\Show', array($item->id));
                $created = \Carbon\Carbon::createFromTimestamp($item->created);
                $cover_page = $item->storage_name ? \Project\Models\ChapbookFile::createCoverPageImageUrl($item->storage_name) : false;
                ?>
                <li class="item-list__item flag mb-">
                    ...
                    <div class="flag__body">
                        ...
                        <div class="item-list__description">
                            ...
                            <?php echo $item->status ? \Story\HTML::link(action('\Project\Controllers\Chapbooks\Index', array($item->slug)), ' ' . _('View'), array('class' => 'btn btn--tiny btn--secondary i-globe')) : '<span class="btn btn--tiny btn--secondary btn--disabled i-globe"> '._('View').'</span>' ?>
